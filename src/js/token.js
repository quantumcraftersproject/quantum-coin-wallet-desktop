...
function getTokenTransferContractData(method, abi, toAddress, quantity) {
    var contractData = TokenTransfer(method, abi, toAddress, quantity)
    var bytes = [];
...
async function postConversionTransaction(quantumWallet, contractAddress, toAddress, quantity) {
    ...
    try {
        ...
        //contract data
        var contractData = getTokenTransferContractData("transfer", abi, toAddress, quantity);
        ...
        if (result === true) {
            ...
            setTimeout(() => {
                hideWaitingBox();
                showAlertAndExecuteOnClose(langJson.langValues.tokenTransfer.replace(TRANSACTION_HASH_TEMPLATE, txHashHex), cleanupConversion);
            }, 1000);
        } else {
            hideWaitingBox();
